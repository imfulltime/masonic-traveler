...
-- Insert badges first (with conflict resolution)
...
ON CONFLICT (id) DO UPDATE SET
  ...
  obfuscated_handle = EXCLUDED.obfuscated_handle,
  role = EXCLUDED.role,
...
ON CONFLICT (lodge_id, user_id) DO NOTHING;
...
-- Populate leaderboards (clear first to avoid conflicts)
DELETE FROM leaderboard_global;
DELETE FROM leaderboard_by_gl;  
DELETE FROM leaderboard_by_district;
